<h1 align="center">Hi there, I'm Meghana!</h1>

<p align="center">
  Engineer + founder | Building AI products end-to-end | Applied ML, full-stack, serverless infra
</p>

<p align="center">
  <a href="https://askabhi.ai"><img src="https://img.shields.io/badge/ASK%20ABHI-000000?style=for-the-badge&logo=probot&logoColor=white" /></a>
  <a href="https://apexlot.vercel.app"><img src="https://img.shields.io/badge/APEXLOT-FF4500?style=for-the-badge&logo=vercel&logoColor=white" /></a>
  <a href="https://facefind-eight.vercel.app"><img src="https://img.shields.io/badge/FACEFIND-1E90FF?style=for-the-badge&logo=icloud&logoColor=white" /></a>
  <a href="https://www.linkedin.com/in/meghana-davuluri-397b37130/"><img src="https://img.shields.io/badge/LINKEDIN-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" /></a>
  <a href="mailto:meghan.davuluri@gmail.com"><img src="https://img.shields.io/badge/EMAIL-D14836?style=for-the-badge&logo=gmail&logoColor=white" /></a>
</p>

---

### About

I'm an engineer and founder building **AI-powered products** from idea to production. My work spans **applied machine learning**, **full-stack development**, and **serverless cloud infrastructure** — with a focus on shipping real products that solve real problems.

I move across the stack: training and deploying ML models, designing APIs, building production UIs, and standing up the cloud infrastructure that holds it all together.

### Currently Shipping

- **[ApexLot](https://apexlot.vercel.app)** — Multi-mode photo search for motorsport and car events. CLIP-based visual search, OCR, color and plate filters. Built on Next.js, Vercel, ONNX, and Replicate.
- **[askabhi.ai](https://askabhi.ai)** — My AI sales agent, live and taking real calls. ElevenLabs Voice + knowledge base + Calendly handoff.
- **[FaceFind](https://facefind-eight.vercel.app)** — Face-recognition photo search at event scale. Migrated from a GCP VM to Cloud Run + Vercel + Upstash for serverless wins.
- **Hooten Young Marketing Hub** — AI-powered competitor intelligence for a premium spirits brand. RAG + multimodal Claude Vision.

### Selected Public Work

- **[Meeting Summary Extractor](https://github.com/Meghana-davuluri/Meeting-Summary-Extractor)** — Whisper + GPT pipeline for meeting summaries.
- **[MDR Bridge](https://github.com/Meghana-davuluri/mdr-bridge-website)** — Medical device consulting site.
- **[ApexLot Case Study](https://github.com/Meghana-davuluri/apexlot-case-study)**
- **[FaceFind Case Study](https://github.com/Meghana-davuluri/facefind-case-study)**

### Stack

<p>
  <img src="https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white" />
  <img src="https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/Java-ED8B00?style=flat&logo=openjdk&logoColor=white" />
  <img src="https://img.shields.io/badge/Next.js-000000?style=flat&logo=nextdotjs&logoColor=white" />
  <img src="https://img.shields.io/badge/FastAPI-009688?style=flat&logo=fastapi&logoColor=white" />
  <img src="https://img.shields.io/badge/Spring%20Boot-6DB33F?style=flat&logo=springboot&logoColor=white" />
  <img src="https://img.shields.io/badge/React-61DAFB?style=flat&logo=react&logoColor=black" />
  <img src="https://img.shields.io/badge/Vercel-000000?style=flat&logo=vercel&logoColor=white" />
  <img src="https://img.shields.io/badge/Cloud%20Run-4285F4?style=flat&logo=googlecloud&logoColor=white" />
  <img src="https://img.shields.io/badge/AWS-232F3E?style=flat&logo=amazonwebservices&logoColor=white" />
  <img src="https://img.shields.io/badge/Upstash-00E9A3?style=flat&logo=upstash&logoColor=white" />
  <img src="https://img.shields.io/badge/Postgres-4169E1?style=flat&logo=postgresql&logoColor=white" />
  <img src="https://img.shields.io/badge/OpenAI-412991?style=flat&logo=openai&logoColor=white" />
  <img src="https://img.shields.io/badge/Anthropic-D97757?style=flat&logo=anthropic&logoColor=white" />
  <img src="https://img.shields.io/badge/ElevenLabs-000000?style=flat&logo=elevenlabs&logoColor=white" />
  <img src="https://img.shields.io/badge/ONNX-005CED?style=flat&logo=onnx&logoColor=white" />
</p>

---

<p align="center">
  <i>"Engineer the boring parts. Ship the magic."</i>
</p>
